...
def send_password_recovery(email):
    ...
    try:
        with smtplib.SMTP("smtp.office.com", 587) as server:
            server.starttls()
...
def on_login_click():
    ...
    if user:
        ...
        subprocess.Popen([r'python', r'C:\Users\yndub\Documents\GitHub\CartoonifyApp\login\Dashboard.py'])
...
login_username_entry = customtkinter.CTkEntry(master=login_frame, width=300, height=40, corner_radius=8, fg_color="#f0f0f0", placeholder_text="Username or Email", font=('Poppins', 16), border_width=2)
login_username_entry.place(relx=0.5, y=140, anchor = tkinter.CENTER)

# Password entry field
login_password_entry = customtkinter.CTkEntry(master=login_frame, width=300, placeholder_text="Password", show="*")
...
def check_password_strength(event = None):
    ...
    password_strength_bar.set(strength / 100)
...
password_strength_bar = customtkinter.CTkProgressBar(master = signup_frame, orient='horizontal', length=200, mode='determinate')
password_strength_bar.place(relx = 0.5, y = 340, anchor = tkinter.CENTER)
password_strength_bar.set(0)
